...
// CSS bundle — inline all @import statements so dist/superhot.css is self-contained
const cssConfig = {
...
if (isWatch) {
  const [cssCtx, jsCtx, preactCtx] = await Promise.all([
    context(cssConfig),
    context(jsConfig),
    context(preactConfig),
  ]);
  await Promise.all([cssCtx.watch(), jsCtx.watch(), preactCtx.watch()]);
