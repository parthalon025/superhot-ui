...
import { ShSkeleton, ShErrorState } from "superhot-ui/preact";
        ...
        {nodes.map((node) => {
          ...
          return (
            ...
            >
              ...
              {/* Primary label — centered or shifted up if showing service stat */}
              <text
                y={stats?.total > 0 ? -2 : 4}
                text-anchor="middle"
                font-family="var(--font-data)"
              ...
              {/* Live service stat — show if project has services */}
              {stats?.total > 0 && (
                <text
                  y={9}
                  text-anchor="middle"
